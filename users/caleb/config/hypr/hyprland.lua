...
-- Animation curves + definitions (Hyprland defaults)
...
hl.bind("Print",         hl.dsp.exec_cmd("grimblast --notify --freeze copysave output ~/Pictures/Screenshots/$(date +%Y-%m-%d_%H-%M-%S).png"))
hl.bind("SHIFT + Print", hl.dsp.exec_cmd("grimblast --notify --freeze copysave area ~/Pictures/Screenshots/$(date +%Y-%m-%d_%H-%M-%S).png"))
hl.bind("ALT + Print",   hl.dsp.exec_cmd("grimblast --notify --freeze copysave active ~/Pictures/Screenshots/$(date +%Y-%m-%d_%H-%M-%S).png"))
...
-- Workspaces: switch (mainMod + N) and move window to workspace (mainMod + SHIFT + N)
for i = 1, 10 do
...
---- WINDOW RULES ----
----------------------
-- Ignore self-maximize requests from apps
hl.window_rule({
